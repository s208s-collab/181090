...
def validate_init_data(init_data: str) -> MiniAppUser:
    ...
    # Newer Telegram clients may include ``signature`` alongside ``hash``.
    # Telegram's HMAC documentation historically described all fields, while
    # newer clients can produce the HMAC before adding that optional signature.
    # Both variants remain protected by the bot-token-derived HMAC.
    signed_variants = [values]
    if "signature" in values:
        signed_variants.append({key: value for key, value in values.items() if key != "signature"})

    is_valid = any(
        hmac.compare_digest(
            hmac.new(
                secret_key,
                "\n".join(f"{key}={variant[key]}" for key in sorted(variant)).encode(),
                hashlib.sha256,
            ).hexdigest(),
            received_hash,
        )
