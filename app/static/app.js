(() => {
  ...
  function sortOrdersByNumber(orders) {
    return [...orders].sort((left, right) => {
      const leftNumber = Number.parseInt(left.order_number || String(left.id), 10);
      const rightNumber = Number.parseInt(right.order_number || String(right.id), 10);
  ...
  async function loadOrders() {
    ...
    try {
      const nextOrders = sortOrdersByNumber(await request("/api/orders"));
